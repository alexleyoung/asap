...
def create_group(db: Session, group: schemas.GroupCreate, owner: models.User):  # Changed parameter name for clarity
    ...
    calendar = db.query(models.Calendar).filter(models.Calendar.id == group.calendarID).first()
    if not calendar:
...
def update_member_permission(
    ...
    membership = db.query(models.Membership).filter(
        models.Membership.groupID == groupID,
        models.Membership.userID == memberID
    ).first()
...
def remove_member(db: Session, groupID: int, memberID: int, adminID: int):
    ...
    # Remove membership
    result = db.query(models.Membership).filter(
        models.Membership.groupID == groupID,
        models.Membership.userID == memberID
    ).delete()
